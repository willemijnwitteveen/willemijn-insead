...
**Your Answers here:**
-quantmod package - financial modelling package
- https://github.com/R-Finance - Github repository where you can find many usefull codes to perform finance calculations
- 
...
- in my dataset the mytickers are defined as "SPY" and "AAPL"
...
- Cumulative: 
' round(100*sum(StockReturns[,2]),1) = 596.5%
- Average daily returns =  0.148%
' round(100*mean(StockReturns[,2]),3) = 0.148%
- Standard deviation
' round(100*sd(StockReturns[,2]),3) = 2.39%
...
**Your Answers here:**
Just add "YHOO" in the command for mytickers. Perform the same commands as we did for Apple, but using [,3] instead of [,2] int the formulas
...
**Your Answers here:**
1. transposeData<-t(Stockreturns) defines the transposed matrix; dim(transposeData) will give the dimensions
(2 4030 or 2 rows, 4030 columns)
2. 
'Transpose the portfolio
> transposedData = t(StockReturns)
'Define the portfolio, using the mean over the columns (the matrix is transposed after all) 
' > portfolioT = apply(StockReturns,2,mean)
Define the names of the x-axis
> names(portfolioT)=rownames(StockReturns)
Plot the portfolio
> pnl_plot(portfolioT)
...
**Your Answers here:**
This is already corectly defined in the dataset:
startDate = "2001-01-01"

2. change mytickers = c("SPY", "AAPL") to mytickers = c("SPY", "AAPL", "YHOO" ), and redo the run
...
**Your Answers here:**
1. The command means: sum if myData in not equal to StockReturns. I would expect to get zero (as myData is a list, and StockReturns is a vector). you get 20, I don't know how to explain it. Maybe because that is the number of arguments in the vector of StockReturn[1:20] and/or the number or rows in the list myData
2. You get an error. As you are 1) trying to sum a list and a vector, and 2) the number of rows is different (20 vs 40)
...
renderPlot({pnl_plot(portfolio[input$startdate:input$enddate])
})
```
